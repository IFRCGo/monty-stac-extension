...
| [monty:country_codes](https://github.com/IFRCGo/monty-stac-extension#montycountry_codes)\[0]                                            | geocode (fallback: reverse-geocoded point)                                                                      | ISO3 code from `geocode`. When GLIDE reports the sentinel `---` (unresolved), the transformer instead reverse-geocodes the event's lat/lon point via `MontyGeoCoder.get_iso3_from_point`; if no geocoder is configured, `monty:country_codes` is an empty list. `keywords` also carries this same ISO3 code, not a human-readable country name                              |
...
There is not straightforward mapping from the GLIDE event type to the [hazard profile](../../taxonomy.md#2025-update). Most GLIDE `event` types resolve to a fixed UNDRR-ISC 2025 / GLIDE / EM-DAT triple in `GlideTransformer.get_hazard_codes`:
...
| **MS** (Mud Slide)             | `GEO-GROUND`                  | `GH0303` (Flows)            |
