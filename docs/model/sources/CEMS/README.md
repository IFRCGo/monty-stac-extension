...
| Volcanic Activity | GH0201 | VO | nat-geo-vol | Eruption (chapeau), always `GH0201` — refining by `subCategory` to ashfall (GH0202) or lahar (GH0204) is not currently implemented |
...
| Transport accident | — | — | tec-tra | Technological — no single chapeau, and **no automatic refinement is implemented**: `transport accident` is in the manual-review set and produces no hazard code until reviewed. TL0401/TL0403/TL0404/TL0405 exist in the taxonomy for air/water/rail/road respectively, but nothing in the transformer currently assigns them |
