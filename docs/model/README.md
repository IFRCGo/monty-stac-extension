...
The full design rationale, framework survey, taxonomy of response type codes, and Sendai Framework crosswalk are documented in [Response Taxonomy](response-taxonomy.md). The extension-layering rules and worked-example combinations are in [Response Best Practices](response-best-practices.md). The rules for deciding, per piece of data, whether something belongs on a Response item, on a paired Impact item, or on neither — the pattern catalogue, decision tree, ETL splitting algorithm, and query patterns — are in [Response ↔ Impact Boundary Rules](response-impact-boundary.md).
...
- **response_detail**: A detailed description of the response including:
  ...
  - **sendai_targets**: Subset of `["A","B","C","D","E","F","G"]` indicating the Sendai Framework targets this response contributes to. See the [response taxonomy](response-taxonomy.md#5-sendai-framework-crosswalk) for the per-type defaults. **OPTIONAL**
