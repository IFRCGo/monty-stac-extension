...
Without arguments, writes:
  - docs/model/sources/README.md — "Available Sources" and "Data Types by
    Source" sections, replaced between AUTOGENERATED markers
...
def main() -> int:
    ...
    if args.check:
        ...
        if new_readme != current_readme:
            drift.append(f"{SOURCES_README.relative_to(ROOT)} is out of date — run scripts/gen_sources_index.py")
        current_sources_json = SOURCES_JSON.read_text(encoding="utf-8") if SOURCES_JSON.exists() else None
        if new_sources_json != current_sources_json:
            drift.append(f"{SOURCES_JSON.relative_to(ROOT)} is out of date — run scripts/gen_sources_index.py")
        if drift:
            _fail("Generated source indexes are stale:", drift)
        print(f"sources.yml, {SOURCES_README.relative_to(ROOT)} and {SOURCES_JSON.relative_to(ROOT)} are in sync "
              f"({len(sources)} sources).")
        return 0

    SOURCES_README.write_text(new_readme, encoding="utf-8")
    SOURCES_JSON.write_text(new_sources_json, encoding="utf-8")
    print(f"Wrote {SOURCES_README.relative_to(ROOT)} and {SOURCES_JSON.relative_to(ROOT)} from "
          f"{len(sources)} sources.")
    return 0
