...
> [!TIP]
> To track which revision of a source transformer produced a given item, declare the [`processing:` extension](https://github.com/stac-extensions/processing) and populate `processing:version` (the transformer's [Semantic Versioning](https://semver.org/) string) and `processing:software` (e.g. `{"pystac-monty": "1.0.0"}`), rather than adding a Monty-specific field — per the [extension layering principle](https://ifrcgo.org/monty-stac-extension/model/response-best-practices.md#1-governing-principles).
