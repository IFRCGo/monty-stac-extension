...
| value         | number | **REQUIRED** The estimated impact value, as a number, without the units. For a probabilistic estimate (see `probability` below), this is the representative value for that value/range (e.g. a bin midpoint)                                                                                  |
| value_min     | number | Lower bound of the estimate, when the source expresses this impact as a range rather than a single value. Only meaningful together with `value_max`                                                                                                                                           |
| value_max     | number | Upper bound of the estimate. Only meaningful together with `value_min`                                                                                                                                                                                                                        |
| probability   | number | Probability (0–1) that the true value falls in [`value_min`, `value_max`]. Present only when the source item is one value/range drawn from a probability distribution rather than a single-point estimate — e.g. one bin of a PAGER alert histogram. Omit for ordinary single-point estimates |
...
| source_id         | string    | Source-system identifier for the response (e.g., CEMS activation code `EMSR744`, Charter VAP id `1144-1`, DREF operation id)                                                                                                                               |
