...
- DesInventar's Hazard Code Mapping table in `docs/model/sources/DesInventar/README.md` rewritten against the actual `hazard_mapping` dict in [`IFRCGo/pystac-monty#201`](https://github.com/IFRCGo/pystac-monty/pull/201) (`fix/desinventar-hazard-mappings`), which grew the transformer's coverage from 25 hand-picked event types to 92, and now documents the two-step resolution the code performs: raw `evento` values (typos, translations, legacy source codes) are first normalized through a 300+ entry alias table (`hazard_name_mappings`) to one canonical event name, which is then looked up in `hazard_mapping` for its `[UNDRR-ISC 2025, EM-DAT, GLIDE]` triplet. Every code in the rewritten table is cross-checked against `docs/model/taxonomy.md` and upstream `HazardProfiles.csv` for its cluster and description, verified by parsing the dict with Python's `ast` module against the exact commit rather than reading it by eye. Six entries (`ACCIDENT`, `BOAT CAPSIZE`, `EPIZOOTIC`, `EXPLOSION`, `LEAK`, `STRUCT.COLLAPSE`) still carry pre-2025 UNDRR-ISC codes that were never migrated — flagged with their 2025 equivalents per taxonomy.md's Cross-Classification Mapping table, each of which already has its own newer, differently-named entry in the same dict. The 50 canonical events with no mapping (`None`, or `OTHER`/`OT - Other`-style catch-alls) are listed explicitly: records with these types, or any raw `evento` matching neither the alias table nor `hazard_mapping`, are dropped entirely — no event, hazard, or impact item is produced. Updated a second time after the PR was revised at [`8778385`](https://github.com/IFRCGo/pystac-monty/commit/8778385f8971078c3b9c414ee3be0a83e9a63304): `BIOLOGICAL` and `Epidemic` were dropped as standalone canonical entries once their raw-string variants were re-aliased to the existing `EPIDEMIC` entry, so both rows were removed from the table rather than left as stale duplicates
