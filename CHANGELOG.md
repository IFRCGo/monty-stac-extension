...
- IFRC DREF source doc corrected against the actual transformer code, closing [etl-drift issue #101](https://github.com/IFRCGo/monty-stac-extension/issues/101) raised after [IFRCGo/pystac-monty#181](https://github.com/IFRCGo/pystac-monty/pull/181) and [#187](https://github.com/IFRCGo/pystac-monty/pull/187). `docs/model/sources/IFRC-DREF/README.md` now documents: impact items generated from **every** field report on an event, not just the first, so an event with *N* field reports and *M* impact types carrying non-null values can produce up to *N* × *M* impact items; the impact item ID format gained a field-report-id suffix (`ifrcevent-impact-{event_id}-{impact_type}-{field_report_id}`) to keep those items unique across reports; geometry generation unions polygons across multiple countries for both event and impact items, with impact items falling back to the parent event's geometry when their own field report carries no usable country; and impact items now carry their own `monty:country_codes` from the field report's `countries[].iso3`, falling back to the event's country codes when the field report has none. `sources.ifrcevent.reviewed` bumped to [`e4fc88b3`](https://github.com/IFRCGo/pystac-monty/commit/e4fc88b3d30c2e7d7d0ac359de6bd24096e972cd) in `.github/etl-watch.yml`. Not fixed here: cross-checking the hazard table against upstream `HazardProfiles.csv` surfaced that `Civil Unrest`'s claimed "no GLIDE or EM-DAT row" is only half right — `taxonomy.md`'s own cross-classification table pairs GLIDE `CE` with `SO0103` (EM-DAT is genuinely absent) — but the transformer doesn't emit `CE` either, so correcting the doc's claim without a matching upstream code change would just relocate the drift; left for a follow-up issue
