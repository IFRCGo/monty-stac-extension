...
- Agent-facing surface: `AGENTS.md` promoted from a stub to a real agent brief (three pillars, repo map, `npm test`, and the do-not-violate invariants — taxonomy-canonical hazard codes, the `monty:corr_id` per-source/not-a-cross-source-join-key caveat consistent with [#57](https://github.com/IFRCGo/monty-stac-extension/issues/57), schema↔README description sync, fixture grounding — resolving the dangling `AGENTS.md` link in the Charter doc); and the `.claude/skills/add-monty-source/` skill (`SKILL.md` driving the five-stage pipeline as an executable workflow, with `references/hazard-code-verification.md`), which *points at* `METHODOLOGY.md`/`SOURCE_TEMPLATE.md` rather than duplicating them. `.claude/settings.local.json` added to `.gitignore` (local agent settings stay local) and `.claude/` excluded from the markdown linter [#70](https://github.com/IFRCGo/monty-stac-extension/issues/70)
